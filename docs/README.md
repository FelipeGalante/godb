...
- **[Usage guide](usage/)** — how to use GoDB today (and where on the roadmap each feature lands). Start here if you want to *run* GoDB rather than read its internals.
...
- **[The development book](book/)** — a chapter-per-milestone narrative explaining the database-internals concepts behind each layer, then walking through the code. Reads from the start of the project to the current milestone.

If you are reading the repo cold, start with the [usage guide](usage/) if you want to know what works today, or the [book introduction](book/00-introduction.md) if you want the engineering narrative from the first commit. The PRD answers *what is being built*; the ADRs answer *why the design looks like this*; the book answers *how would I learn to build this myself*; the usage guide answers *what can I do with GoDB right now and where does each feature land*.
...
- **A top-level [README](../README.md) update** — tick the milestone in the Roadmap, refresh the *Project status* paragraph if its accuracy changed, and tweak the *What you can do today* paragraph if a new user-visible feature landed.
